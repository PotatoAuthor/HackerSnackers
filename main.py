...
def get_free_time(cals_to_check, service, calendars_dict, max_scan_time_days):
        ...
        schedule_array = [0]*(max_scan_time_days*96) 
        ...
        max_search_timestamp = str((datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(days = max_scan_time_days)).isoformat()[:-6] + 'Z')

        # this convoluted line of code finds the exact timestamp of the next [LOCAL TIME]
...
def main():
    ...
    try:
        ...
        for key in calendars_dict:
            print(key, ' : ', calendars_dict[key])

        cals_to_check = ['golem8gamer@gmail.com', 'Holidays in Canada', 'broland']
        max_scan_time_days = 1
        free_time = get_free_time(cals_to_check, service, calendars_dict, max_scan_time_days)
